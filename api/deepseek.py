...
class DeepSeekClient:
    ...
    def analyze_causal_data(self, prompt):
        """分析因果数据的方法"""
        try:
            prompt_sys = f"""你是一位教育数据分析专家。请用**通俗易懂的语言**向学校管理者解释这份因果推断报告的核心发现，需满足：
                1. 说清核心因果关系（X→Y的影响）
                2. 说明结论可靠度（用日常语言解释p值）
                3. 给出可直接执行的教育建议
                ...
                • X：每周课后辅导次数  
                • Y：数学期末成绩  
                • 因果效应：每增加1次辅导，成绩提高0.8分（p<0.01）  
                • 干扰因素：学生家庭资源（高资源家庭效果减半）

                ​**你的回答模板**​：
                ▸ ​**核心发现**​：当[原因变量]时，会导致[结果变量][具体变化]。相当于老师能观察到的效果是：______（比喻说明）  
                ▸ ​**可信度**​：这个结论有[用日常词描述p值]的把握，例如______  
                ▸ ​**行动建议**​：优先对[具体人群]实施[具体措施]，因为______"""
            messages = [
                {
                    "role": "system",
                    "content": prompt_sys
